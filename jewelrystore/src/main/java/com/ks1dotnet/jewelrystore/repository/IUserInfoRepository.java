...
public interface IUserInfoRepository extends JpaRepository<UserInfo, Integer> {
        ...
        @Query(value = "SELECT * FROM user_info u WHERE u.id_role = 4", countQuery = "SELECT count(*) FROM user_info u WHERE u.id_role = 4", nativeQuery = true)
        Page<UserInfo> findCustomersByRoleId(Pageable pageable);

        @Query(value = "SELECT * FROM user_info u WHERE u.id_role = 5", countQuery = "SELECT count(*) FROM user_info u WHERE u.id_role = 5", nativeQuery = true)
