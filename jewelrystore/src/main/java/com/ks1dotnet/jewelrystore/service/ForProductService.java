...
public class ForProductService implements IPromotionGenericService<Product> {
                ...
                try {
                        ...
                        for (Product product : products) {
                                List<ForProduct> activePromotions = iForProductRepository
                                                .findActiveProductPromotionsByProductIdAndInvoiceTypeId(
                                                                product.getId(),
                                ...
                                ForProduct existingForProduct =
                                                iForProductRepository.findByPromotionIdAndProductId(
                                                                promotionId, product.getId());
                                if (existingForProduct != null) {
                                        existingForProduct.setStatus(true);
                                        forProductsToSave.add(existingForProduct);
                                } else {
                                        forProductsToSave.add(
                                                        new ForProduct(promotion, product, true));
                                }
                        }

                        iForProductRepository.saveAll(forProductsToSave);
                        List<ForProductDTO> forProductDTOs = forProductsToSave.stream()
                                        .map(ForProduct::getDTO).collect(Collectors.toList());
                        ...
                        throw new ApplicationException(
                                        "Error at applyPromotion ForProductService: "
                                                        + e.getMessage(),
        ...
        public ResponseData removePromotion(ApplyPromotionDTO applyPromotionDTO) {
                try {
                        ...
                        return new ResponseData(HttpStatus.OK,
                                        "Promotion removed from products successfully", null);
                } catch (ApplicationException e) {
                        throw new ApplicationException(
                                        "Error at removePromotion ForProductService: "
                                                        + e.getMessage(),
                                        e.getErrorString(), e.getStatus());
                } catch (Exception e) {
                        throw new ApplicationException(
        ...
        public ResponseData checkInOtherActivePromotions(int entityId, int promotionId) {
                try {
                        ...
                        if (forProducts.isEmpty()) {
                                return new ResponseData(HttpStatus.OK,
                                                "Product is not in any other active promotions",
                                                null);
                        }
                        ...
                        if (otherPromotions.isEmpty()) {
                                ...
                                return new ResponseData(HttpStatus.CONFLICT,
                                                "Product is active in other promotions",
                                                otherPromotions);
                        }
                } catch (ApplicationException e) {
                        throw new ApplicationException(
                                        "Error at checkInOtherActivePromotions ForProductService: "
                                                        + e.getMessage(),
                                        e.getErrorString(), e.getStatus());
                } catch (Exception e) {
                        throw new ApplicationException(
        ...
        public ResponseData getEntitiesNotInPromotion(int promotionId) {
                try {
                        ...
                        if (products == null || products.isEmpty()) {
                                throw new ApplicationException(
                                                "No products found not in the given promotion id: "
                                                                + promotionId,
                                                HttpStatus.NOT_FOUND);
                        }

                        List<ProductDTO> productDTOs = products.stream().map(Product::getDTO)
                                        .collect(Collectors.toList());

                        return new ResponseData(HttpStatus.OK,
                                        "Products not in promotion found successfully",
                                        productDTOs);
                } catch (ApplicationException e) {
                        throw new ApplicationException(
                                        "Error at getEntitiesNotInPromotion ForProductService: "
                                                        + e.getMessage(),
                                        e.getErrorString(), e.getStatus());
                } catch (Exception e) {
                        throw new ApplicationException(
                ...
                try {
                        ...
                        if (forProducts == null || forProducts.isEmpty()) {
                                return new ResponseData(HttpStatus.BAD_REQUEST,
                                                "No products found for the given promotion id: "
                                                                + promotionId,
                                                null);
                        ...
                        throw new ApplicationException(
                                        "Error at getEntitiesInPromotion ForProductService: "
                                                        + e.getMessage(),
                                        e.getErrorString(), e.getStatus());
                } catch (Exception e) {
                        throw new ApplicationException(
