...
public class EarnPointsService implements IEarnPointsService {
    ...
    public ResponseData getAllEarnPoints() {
        ...
        try {
            List<EarnPointsDTO> earnPointsDTOList = iEarnPointsRepository.findAll().stream()
                    .map(earnPoints -> earnPoints.getDTO(new EarnPointsDTO())) // Adjusted to use
                                                                               // the updated DTO
                                                                               // method
    ...
    public void updateCustomerTypeBasedOnPoints(Integer customerId) {
        ...
        if (earnPoints != null) {
            ...
            CustomerType newCustomerType = customerTypes.stream()
                    .filter(ct -> totalPoints >= ct.getPointCondition()).max((ct1, ct2) -> Integer
                            .compare(ct1.getPointCondition(), ct2.getPointCondition()))
                    .orElse(null);
    ...
    public ResponseData addPoints(Integer customerId, Integer points) {
        Optional<EarnPoints> optionalEarnPoints =
                iEarnPointsRepository.findByCustomerId(customerId);
        EarnPoints earnPoints;
        if (optionalEarnPoints.isPresent()) {
            earnPoints = optionalEarnPoints.get();
            earnPoints.setPoint(earnPoints.getPoint() + points);
        } else {
            UserInfo userInfo = iUserInfoRepository.findById(customerId).orElseThrow(
                    () -> new ApplicationException("Customer not found", HttpStatus.NOT_FOUND));
            ...
            earnPoints.setCustomerType(iCustomerTypeRepository.findById(1)
                    .orElseThrow(() -> new ApplicationException("Not found customer type",
        ...
        EarnPointsDTO earnPointsDTO =
                new EarnPointsDTO(savedEarnPoints.getId(), savedEarnPoints.getPoint(),
