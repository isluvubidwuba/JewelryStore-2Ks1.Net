...
@RestController
@RequestMapping("/promotion-generic")
@CrossOrigin("*")
public class GenericPromotionController {
    ...
    @PostMapping("/apply")
    public ResponseEntity<ResponseData> applyPromotion(@RequestBody ApplyPromotionDTO applyPromotionDTO) {
        ResponseData responseData = genericPromotionService.applyPromotion(applyPromotionDTO);
        return new ResponseEntity<>(responseData, responseData.getStatus());
    }

    @PostMapping("/remove")
    public ResponseEntity<ResponseData> removePromotion(@RequestBody ApplyPromotionDTO applyPromotionDTO) {
    ...
    public ResponseEntity<ResponseData> checkEntityInOtherPromotions(@PathVariable EntityType entityType,
            @PathVariable int entityId,
            @PathVariable int promotionId) {
        ResponseData responseData = genericPromotionService.checkEntityInOtherPromotions(entityType, entityId,
                promotionId);
        return new ResponseEntity<>(responseData, HttpStatus.OK);
    }

    @GetMapping("/not-in-promotion/{entityType}/{promotionId}")
    public ResponseEntity<ResponseData> getEntitiesNotInPromotion(@PathVariable EntityType entityType,
            @PathVariable int promotionId) {
        ResponseData responseData = genericPromotionService.getEntitiesNotInPromotion(entityType, promotionId);
        return new ResponseEntity<>(responseData, responseData.getStatus());
    }
    ...
    public ResponseEntity<ResponseData> getEntitiesInPromotion(@PathVariable EntityType entityType,
            @PathVariable int promotionId) {
        ResponseData responseData = genericPromotionService.getEntitiesInPromotion(entityType, promotionId);
        return new ResponseEntity<>(responseData, responseData.getStatus());
    }
}
