$(document).ready(function () {
    ...
    $('#searchBtn').click(function () {
        ...
        if (barcode.trim() !== '' || selectedInvoiceType !== '') {
            ...
            alert('Please select an invoice type or enter a product barcode before searching.');
        }
    });
            ...
            return `
                <div class="w-1/2 px-2 flex product-info fade-in">
                    ...
                    <div class="w-4/12 p-2">
                        <img src="${data.productDTO.imgPath}" alt="Product Image" class="max-w-full h-auto rounded">
                        <div class="mt-2 space-y-1">
                            <span class="tag">Status: ${data.productDTO.status ? 'Active' : 'Inactive'}</span>
                            <span class="tag">Weight: ${data.productDTO.weight}g</span>
                            <span class="tag">Material: ${data.productDTO.materialDTO.name}</span>
                            <span class="tag">Purity: ${data.productDTO.materialDTO.purity}</span>
                            <span class="tag">Category: ${data.productDTO.productCategoryDTO.name}</span>
                        </div>
                    </div>
                    <div class="w-8/12 p-2">
                        <p class="text-lg font-semibold">Product Code: ${data.productDTO.productCode}</p>
                        <p class="text-lg font-semibold">Bar Code: ${data.productDTO.barCode}</p>
                        <p class="text-lg font-semibold">Name: ${data.productDTO.name}</p>
                        <p class="text-lg font-semibold">Fee: ${formatCurrency(data.productDTO.fee)}</p>
                        <p class="text-lg font-semibold">Price at Time: ${formatCurrency(data.productDTO.materialDTO.priceAtTime)}</p>
                        <p class="text-lg font-semibold">Counter: ${data.productDTO.counterDTO.name}</p>
                        <p class="text-lg font-semibold">Inventory Quantity: ${data.productDTO.inventoryDTO.quantity}</p>
                        <p class="text-lg font-semibold">Price: ${formatCurrency(data.price)}</p>
                        <p class="text-lg font-semibold">Total Price: ${formatCurrency(data.totalPrice)}</p>
                    </div>
                </div>
            `;
        } else {
